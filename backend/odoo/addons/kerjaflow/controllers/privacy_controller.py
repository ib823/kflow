...
class PrivacyController(http.Controller):
    ...
    def request_data_access(self, **kwargs):
        ...
        if not employee:
            return {
                "success": False,
                "error": {"code": "NO_EMPLOYEE", "message": "No employee record found"},
            }

        # Create data access request
        data_request = (
            request.env["kf.privacy.request"]
            .sudo()
            .create(
                {
                    "user_id": user.id,
    ...
    @http.route(
        "/api/v1/privacy/delete-request/<string:token>",
        type="json",
        auth="public",
        methods=["GET"],
        csrf=False,
    ...
    @http.route(
        "/api/v1/privacy/delete-confirm/<string:token>",
        type="json",
        auth="public",
        methods=["POST"],
        csrf=False,
    ...
    @http.route(
        "/api/v1/privacy/download/<string:token>",
        type="http",
        auth="public",
        methods=["GET"],
        csrf=False,
